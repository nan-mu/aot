...
static void scalar_byte_swap(struct bpf_reg_state *dst_reg, struct bpf_insn *insn)
{
	...
	if (need_bswap) {
		 ...
		 */
		inner_mark_reg_unbounded(dst_reg);
	}
			       ...
			       struct bpf_reg_state *src_reg)
{
	...
	if (src_known && dst_known) {
		inner_mark_reg_known(dst_reg, dst_reg->var_off.value);
		return;
	}
	...
	inner_update_reg_bounds(dst_reg);
}
			       ...
			       struct bpf_reg_state *src_reg)
{
	...
	inner_scalar64_min_max_lsh(dst_reg, umin_val, umax_val);
	inner_scalar32_min_max_lsh(dst_reg, umin_val, umax_val);
...
// Extracted from /Users/nan/bs/aot/src/verifier.c
static void inner_scalar32_min_max_lsh(struct bpf_reg_state *dst_reg,
...
// Extracted from /Users/nan/bs/aot/src/verifier.c
static void inner_scalar64_min_max_lsh(struct bpf_reg_state *dst_reg,
				   u64 umin_val, u64 umax_val)
{
				 ...
				 struct bpf_reg_state *src_reg)
{
	...
	if (src_known && dst_known) {
		inner_mark_reg32_known(dst_reg, var32_off.value);
		return;
	}
				  ...
				  struct bpf_reg_state *src_reg)
{
	...
	inner_mark_reg64_unbounded(dst_reg);
	inner_update_reg32_bounds(dst_reg);
}
